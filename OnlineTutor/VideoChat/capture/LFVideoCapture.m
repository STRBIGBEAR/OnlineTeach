...
@property (nonatomic, strong) CustomGPUImageFilter *lookUpFilter;
...
- (instancetype)initWithVideoConfiguration:(LFLiveVideoConfiguration *)configuration {
    if (self = [super init]) {
        ...
        self.beautyFace = YES;
        self.beautyLevel = 0.9;
        self.brightLevel = 0.9;
        self.zoomScale = 1.0;
        self.mirror = YES;
    }
    return self;
...
- (CustomGPUImageFilter *)lookUpFilter
{
    if (!_lookUpFilter) {
        _lookUpFilter = [[CustomGPUImageFilter alloc]initWithImage:[UIImage imageNamed:@"fennen.png"]];
    }
...
- (void)processVideo:(GPUImageOutput *)output time:(CMTime)time{
    __weak typeof(self) _self = self;
    @autoreleasepool {
        GPUImageFramebuffer *imageFramebuffer = output.framebufferForOutput;
        
        CVPixelBufferRef pixelBuffer = [imageFramebuffer pixelBuffer];
        if (pixelBuffer && _self.delegate && [_self.delegate respondsToSelector:@selector(captureOutput:pixelBuffer:time:)]) {
            [_self.delegate captureOutput:_self pixelBuffer:pixelBuffer time:time];
        }
    }
}
...
- (void)reloadFilter{
    ...
    if([self.configuration.avSessionPreset isEqualToString:AVCaptureSessionPreset640x480]){
        ...
        /*
         // 响应链
         [self.videoCamera addTarget:self.lookUpFilter];
         [self.lookUpFilter addTarget:self.filter];
    ...
    [self.output setFrameProcessingCompletionBlock:^(GPUImageOutput *output, CMTime time) {
        [_self processVideo:output time:time];
    }];
